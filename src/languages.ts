...
const files = fs.readdirSync(path.join(constants_1.paths.nodeModules, '/timeago/locales'));
exports.Languages.timeagoCodes = files.filter((f) => f.startsWith('jquery.timeago')).map((f) => f.split('.')[2]);
...
exports.Languages.userTimeagoCode = function (userLang) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (languageCodes.includes(userLang) && exports.Languages.timeagoCodes.includes(timeagoCode)) {
            return timeagoCode;
        }
        return '';
    });
